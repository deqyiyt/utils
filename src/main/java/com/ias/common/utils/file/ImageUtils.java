...
public class ImageUtils {
    ...
    public static void main(String[] args) {
        ...
        // 5-给图片添加文字水印：
        // 方法一：
        ImageUtils.pressText("我是水印文字","e:/abc.jpg","e:/abc_pressText.jpg", Color.white, new Font("宋体", Font.BOLD, 100), 0, 0, 0.5f, null);//测试OK
        
        // 6-给图片添加图片水印：
        ImageUtils.pressImage("e:/abc2.jpg", "e:/abc.jpg","e:/abc_pressImage.jpg", 0, 0, 0.5f, null);//测试OK
    }
    ...
    public final static void pressText(String pressText
        ...
        try {
            ...
            g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP,
                    alpha));
            // 在指定坐标绘制水印文字
            g.drawString(pressText, (width - (getLength(pressText) * font.getSize()))
     ...
     * @param degree 倾斜度，如：倾斜45度 -45
     */
    public final static byte[] pressText(String pressText, byte[] srcImageData
            , Color color,Font font
            , int x, int y, float alpha, Integer degree) {
        try {
            Image src = ImageUtils.bytesToImage(srcImageData);
            int width = src.getWidth(null);
            int height = src.getHeight(null);
            BufferedImage image = new BufferedImage(width, height,
                    BufferedImage.TYPE_INT_RGB);
     ...
     * 给图片添加图片水印
     * @param pressImg 水印图片
     * @param srcImageData 源图像地址
     * @param x 修正值。 默认在中间
     * @param y 修正值。 默认在中间
     * @param alpha 透明度：alpha 必须是范围 [0.0, 1.0] 之内（包含边界值）的一个浮点数字
     * @param degree 倾斜度，如：倾斜45度 -45
     */
    public final static byte[] pressImage(String pressImg, byte[] srcImageData,
            int x, int y, float alpha, Integer degree) {
        try {
        		Image src = ImageUtils.bytesToImage(srcImageData);
            int wideth = src.getWidth(null);
